...
```{r}
# GET("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed&id=32480061&retmode=json") %>% 
    a <- jsonlite::fromJSON("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=pubmed&id=32480061&retmode=json")$result %>% 
    '[['('32480061') %>% 
    '[['('articleids') %>% 
    filter(idtype == 'doi') %>% 
    pull(value)
...
spacy_initialize(model = "en_ner_jnlpba_md", condaenv = 'spacy-proc')
```
